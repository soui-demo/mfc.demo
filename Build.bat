...
if %specs%==win32-msvc2017 (	
	%SOUIPATH%\tools\qmake2017 -tp vc -r -spec %SOUIPATH%\tools\mkspecs\%specs% "CONFIG=%cfg% DEBUG|RELEASE"
) else (
	%SOUIPATH%\tools\qmake -tp vc -r -spec %SOUIPATH%\tools\mkspecs\%specs% "CONFIG=%cfg% DEBUG|RELEASE"
)
